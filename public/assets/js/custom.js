(function ($) {
    ...
    $('.add-button.add-media').on("click",function(event){
        //console.log($(".medias").length);
        let max_media = $(".medias").length;
        console.log(max_media);
        if (max_media < 5) {
            var newMajesticItem= '<li style="display: none">' +
                ...
                '</div>'+
                '</li>';
            $( '.list-sortable.medias-list').append( newMajesticItem );
            $( '.list-sortable.medias-list').children("li").slideDown();
            bindMajesticItem();
            /*if (max_media == 4) {
                var x = document.getElementsByClassName('add-media')[0];
    ...
    /*function trashed() {
        let media_trash = document.querySelectorAll('.media-trash');
        media_trash.forEach(el => {
            console.log('LOG')
